...
struct SHEET{
	unsigned char *buf; //图层上描画的内容的地址
	int bxsize,bysize, vx0,vy0, col_inv,height,flags; //图层整体大小，图层在画面上的位置，颜色，高度
}; //整个结构体32字节

struct SHTCTL{
	unsigned char *vram;  //vram地址
	int xsize,ysize,top;  //画面的大小，最上面图层的高度
	struct SHEET *sheets[MAX_SHEETS];  //用于存储排好序的图层地址
	struct SHEET sheets0[MAX_SHEETS];  //用于存储每个图层的具体信息
};   //整个结构体9232字节
